    ...
    <section id="certifications" className="section">
      <div className="container">
        ...
        <div className="certifications-container">
          {categorizedCertifications.map((categoryBlock, catIndex) => (
            <div key={catIndex} className={`certification-category animate-fade-in delay-${(catIndex + 1) * 100}`}>
              <h3 className="category-title text-gradient">{categoryBlock.category}</h3>
              <div className="certifications-grid">
                {categoryBlock.items.map((cert, index) => (
                  <div 
                    key={index} 
                    className="cert-card glass-panel hover-glow"
                  >
